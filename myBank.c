...
void openAcount (double amount) {
    ...
    for (int i=0; i<AR_R;i++) {
        ...
        if (bankAc[i][0] == 0) {
            ...
            printf("Error has been made, To open annother account please press E");
        }
        break;
    }
    if (count == AR_R)
...
void balance (int acNum) {
    if (scanf("%d", &acNum) == 1) {
        if ( acNum < AC_MIN || acNum > AC_MAX)
            printf("Error has been made, wrong account number");
        else {
            acNum = acNum - AC_MIN;
            if (bankAc[acNum][0] == 0) {
                printf("Error has been made, the account is closed");
            }
            else {
...
void printAc() {
    for (int i=0; i<AR_R;i++) {
        if (bankAc[i][0] == 1) {
            printf("The balance of the account number  %d is : %0.2d\n",AC_MIN+i,bankAc[i][1]);
        }
    }
}
